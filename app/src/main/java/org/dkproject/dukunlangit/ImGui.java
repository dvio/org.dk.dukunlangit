...
//                    init(surfaceHolder.getSurface(), Utils.sp2px(14), Utils.dp2px(1.2f),
//                            LangitApplication.getLangitResources().getAssets(),
...
//    }
    ...
    public void surfaceCreated(@NonNull SurfaceHolder surfaceHolder) {
        thread.pushRunnable(()->{
            if(thread.hasInitialized) {
                ...
                thread.hasInitialized = true;
                init(surfaceHolder.getSurface(), Utils.sp2px(14), Utils.dp2px(1.2f), LangitApplication.getLangitResources().getAssets(), Locale.getDefault().getLanguage().startsWith("zh"));
            }
        });
    }
    ...
    static class ImGUIThread extends Thread {
        ...
        public void run() {
            try {
                while (!isInterrupted()) { // Stop when interrupted
                    runnables.take().run();
                }
            } catch (InterruptedException e) {
                // Thread is stopped, exit gracefully
            }
        }
